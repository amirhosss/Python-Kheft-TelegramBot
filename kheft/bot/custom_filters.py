...
class IsMember(SimpleCustomFilter):
    ...
    async def check(self, message: Message):
        try:
            chat_memebr = await self._bot.get_chat_member(
                chat_id=configs.telegrambot_public_channel, user_id=message.chat.id
            )
            member_status = chat_memebr.status

            return member_status not in ["left", "banned"]
        except Exception as e:
            print(f"an error occurred during `is_member` by: {e}")
            return False
